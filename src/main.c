...
void* handle_conn(handle_args_t* args) {
  ...
  printf("   \033[0;36m[web]\033[0m :: handling connection on socket %d\n", fd);
...
END_SOCK:
  ...
  if (epoll_ctl(epfd, EPOLL_CTL_DEL, fd, NULL))
    die("failed to remove fd from epoll interest list");
  close(fd);
  return NULL;
}
...
int main(void) {
  ...
  struct epoll_event listen_ev = { .data.fd = sfd,
                                   .events = EPOLLIN | EPOLLET };
  struct epoll_event inotify_ev = { .data.fd = ifd,
                                    .events = EPOLLIN | EPOLLET };
  ...
  for (;;) {
    ...
    for (int e = 0; e < evs; e++) {
      ...
      if (event.data.fd == sfd) {
        int newfd = accept4(sfd, NULL, NULL, SOCK_NONBLOCK);
        ...
        if (epoll_ctl(epfd, EPOLL_CTL_ADD, newfd, &accepted_ev)) {
          ...
          printf(
              "   \033[0;36m[web]\033[0m :: got new connection, adding fd %d "
              "to epoll interest list\n",
              newfd);
        }
        ...
        handle_args_t arg = { .fd = event.data.fd,
                              .config = config,
                              .epfd = epfd };
